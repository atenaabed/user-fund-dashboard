...
axios.get(apiUrl, {
  ...
  .then(response => {
    ...
    worksheet.columns = [
      ...
      { header: 'Slippage', key:'PL', width:20},
      //{ header: 'Profit or Loss(M)', key:'PLM', width:20},
    ...
    historyList.forEach(transaction => {
      ...
      // calculate Receive Price in US currency
      ...
      // calculate Sends Price in US currency
      ...
      const PL = getPriceR - getPriceS ;

      //get number month
      const getMonth = new Date(transaction.time_at  * 1000).toLocaleString('default',{ month: 'numeric' })
      const listOfData = []
      listOfData.push((getMonth))
      listOfData.push(getPriceR)
      listOfData.push(getPriceS)
